import React, { useState, useEffect } from "react";
import ModalListDeliveries from "../Modals/ModalListDeliveries";
import { SortListNumber, SortListUsersByCriteria } from "../Functions/SortList";
import ModalAddDelivery from "../Modals/ModalAddDelivery";
import { SearchItemsDelivery } from "../Functions/SearchItems";
import ReactPaginate from "react-paginate";
import dataTest from "../../../DataTest/dataTestDeliveries";
import "./CSS/style.css";

export default function ListDeliveries() {
  const [selectedDelivery, setSelectedDelivery] = useState(null);
  const [refresh, setRefresh] = useState(false);
  const [ascendant, setAscendant] = useState(false);
  const [catergory, setCatergory] = useState(null);
  const [openModal, setOpenModal] = useState(false);
  const [openModalAdd, setOpenModalAdd] = useState(false);
  const [currentPage, setCurrentPage] = useState(0);
  const usersPerPage = 15;
  const [data, setData] = useState(dataTest);
  const [selectedPersonBySearch, setSelectedPersonBySearch] = useState(data);

  const handleClickSelectedDelivery = (idDelivery) => {
    setSelectedPersonBySearch((prevData) =>
      prevData.map((delivery) =>
        delivery.commandNumber === idDelivery
          ? { ...delivery, is_checked: !delivery.is_checked }
          : delivery
      )
    );
  };

  //Afficher la modale de la livraison selectionnée
  const handleClickDisplayDetails = (delivery) => {
    setSelectedDelivery(delivery);
    setOpenModal(!openModal);
  };

  //Rechercher une livraison
  const handleSearchItems = (e) => {
    if (e.target.value === "") {
      setRefresh(true);
    } else {
      setCurrentPage(0);
      setSelectedPersonBySearch(SearchItemsDelivery(e.target.value, data));
    }
  };

  const handleClick = (key) => {
    setCatergory(key);
    setAscendant(!ascendant);
    setSelectedPersonBySearch(SortListUsersByCriteria(data, ascendant, key));
  };

  // Fonction pour récupérer les utilisateurs de la page actuelle
  const getCurrentOrders = () => {
    const startIndex = currentPage * usersPerPage;
    const endIndex = startIndex + usersPerPage;
    return selectedPersonBySearch.slice(startIndex, endIndex);
  };

  // Fonction appelée lorsque l'utilisateur change de page
  const handlePageChange = (selectedPage) => {
    setCurrentPage(selectedPage.selected);
  };

  useEffect(() => {
    try {
      setSelectedPersonBySearch(SortListNumber(data));
      setRefresh(false); //Trie sur le nom
    } catch (error) {
      console.log("Error with axios: ", error);
    }
  }, [refresh]);

  return (
    <div className="overflow-x-auto w-11/12 m-auto">
      <div className="flex justify-between items-center">
        <div className="flex my-4">
          <h1 className="font-bold mr-4">Liste des livraisons</h1>
          <i
            className="fa-solid fa-plus border border-gray-400 p-1 rounded-full text-halo ease-in-out duration-200 hover:scale-110 cursor-pointer"
            onClick={() => setOpenModalAdd(!openModalAdd)}
          ></i>
          {openModalAdd && <ModalAddDelivery onClose={setOpenModalAdd} />}
        </div>
        <div>
          <input
            type="text"
            placeholder="Search"
            className="w-full text-center rounded-xl bg-gray-400 placeholder:text-gray-800 outline-none p-1 text-white"
            onChange={handleSearchItems}
          />
        </div>
      </div>
      <table className="table">
        {/* head */}
        <thead>
          <tr>
            <th>Selection</th>
            <th
              onClick={(e) => handleClick("commandNumber")}
              className={
                ascendant && catergory === "commandNumber"
                  ? "font-bold text-orange-500 cursor-pointer"
                  : "cursor-pointer"
              }
            >
              N° de commande{" "}
              <i
                className={
                  ascendant
                    ? "fa-solid fa-arrow-down-9-1 ml-2"
                    : "fa-solid fa-arrow-down-1-9 ml-2"
                }
              ></i>
            </th>
            <th
              onClick={(e) => handleClick("lastNameDeliverer")}
              className={
                ascendant && catergory === "lastNameDeliverer"
                  ? "font-bold text-orange-500 cursor-pointer"
                  : "cursor-pointer"
              }
            >
              Nom du livreur{" "}
              <i
                className={
                  ascendant
                    ? "fa-solid fa-arrow-down-z-a ml-2"
                    : "fa-solid fa-arrow-down-a-z ml-2"
                }
              ></i>
            </th>
            <th
              onClick={(e) => handleClick("firstNameDeliverer")}
              className={
                ascendant && catergory === "firstNameDeliverer"
                  ? "font-bold text-orange-500 cursor-pointer"
                  : "cursor-pointer"
              }
            >
              Prénom du livreur{" "}
              <i
                className={
                  ascendant
                    ? "fa-solid fa-arrow-down-z-a ml-2"
                    : "fa-solid fa-arrow-down-a-z ml-2"
                }
              ></i>
            </th>
            <th
              onClick={(e) => handleClick("status")}
              className={
                ascendant && catergory === "status"
                  ? "font-bold text-orange-500 cursor-pointer"
                  : "cursor-pointer"
              }
            >
              Statut{" "}
              <i
                className={
                  ascendant
                    ? "fa-solid fa-arrow-down-z-a ml-2"
                    : "fa-solid fa-arrow-down-a-z ml-2"
                }
              ></i>
            </th>
            <th></th>
            <th></th>
          </tr>
        </thead>
        <tbody>
          {getCurrentOrders() &&
            getCurrentOrders().map((delivery) => (
              <tr key={delivery.commandNumber}>
                <td>
                  <label>
                    <input
                      type="checkbox"
                      className="checkbox"
                      onClick={() => {
                        handleClickSelectedDelivery(delivery.commandNumber);
                      }}
                    />
                  </label>
                </td>
                <td>{delivery.commandNumber}</td>
                <td>{delivery.deliverer.lastNameDeliverer}</td>
                <td>{delivery.deliverer.firstNameDeliverer}</td>
                <td>{delivery.status}</td>
                <td>
                  <button
                    className="btn btn-ghost btn-xs"
                    onClick={() => {
                      handleClickDisplayDetails(delivery);
                    }}
                  >
                    details
                  </button>
                </td>
                <td>
                  {delivery.is_checked && (
                    <i className="fa-solid fa-trash hover:text-red-500 text-xl ease-in-out duration-200 hover:scale-125 cursor-pointer"></i>
                  )}
                </td>
                {openModal &&
                  selectedDelivery.commandNumber === delivery.commandNumber && (
                    <ModalListDeliveries
                      onClose={setOpenModal}
                      delivery={selectedDelivery}
                    />
                  )}
              </tr>
            ))}
        </tbody>
        {/* foot */}
        <tfoot>
          <tr>
            <th>Selection</th>
            <th>N° de commande</th>
            <th>Nom du livreur</th>
            <th>Prénom du livreur</th>
            <th>Statut</th>
            <th></th>
            <th></th>
          </tr>
        </tfoot>
      </table>
      <ReactPaginate
        pageCount={Math.ceil(selectedPersonBySearch.length / usersPerPage)} // Calculer le nombre total de pages
        pageRangeDisplayed={3}
        marginPagesDisplayed={1}
        onPageChange={handlePageChange}
        containerClassName={"pagination"}
        activeClassName={"active"}
        className="m-auto w-1/2 flex justify-evenly items-center mt-5"
      />
    </div>
  );
}
