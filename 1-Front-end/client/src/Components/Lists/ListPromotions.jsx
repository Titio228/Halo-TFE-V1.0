...
export default function ListPromotions() {
  ...
  const handleClickSelectedPromotion = (idPromotion) => {
    setSelectedPersonBySearch((prevData) =>
      prevData.map((promotion) =>
        promotion.id === idPromotion
          ? { ...promotion, is_checked: !promotion.is_checked }
          : promotion
      )
    );
  };

  //Afficher la modale de la promotion selectionnée
  ...
  // Fonction pour récupérer les promotions de la page actuelle
    ...
    <div className="overflow-y-auto w-11/12 m-auto">
      <div className="flex justify-between items-center">
        <div className="flex my-4">
          <h1 className="font-bold mr-4">Liste des promotions</h1>
          <i
                ...
                <td>
                  <label>
                    <input
                      ...
                      className="checkbox"
                      onClick={() => {
                        handleClickSelectedPromotion(promotion.id);
                      }}
                    />
                  </label>
                </td>
                <td>{promotion.name}</td>
                <td>{format(promotion.startDate, "dd-MM-yyyy")}</td>
                <td>{format(promotion.endDate, "dd-MM-yyyy")}</td>
                <td>
                  <i
                    className={
                      promotion.status
